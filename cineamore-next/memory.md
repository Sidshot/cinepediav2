...
## 🚀 NEW DEPLOYMENT GUIDE (Single Repo)

**We have restructured the repository to remove the nested folder confusion.**

### ✅ How to Deploy (The Only Way)

1.  Open Terminal.
2.  Navigate to the repository root: `D:\CinePedia - IDL\CinePedia`.
...
**❌ Do NOT:**
-   Do NOT push to `CinePedia` (old repo).
-   Do NOT look for a `.git` folder inside `cineamore-next/` (it is gone).
-   Do NOT worry about secrets (they are scrubbed).

---

## 2026-01-27: Repository Restructure & Secret Scrubbing

**Session Duration**: ~1 hour
**Status**: ✅ Complete & Deployed
**Critical Fixes**: Single Repository Structure, Secret Removal

### 🎯 Key Outcomes

1.  **Single Repository**: Deleted the nested `.git` directory in `cineamore-next` that was causing `git status` to ignore changes. Consolidated everything into the root `CinePedia` repository.
2.  **Secret Scrubbing**: Identified `import.env` containing Google OAuth keys in history. Used `git reset --soft` to squash commits and permanently remove the file from git history to satisfy GitHub Push Protection.
3.  **Deployment Success**: Force pushed the clean, unified structure to `main`.

---

## 2026-01-27: FAB Glassmorphism & BMC Integration Polish

**Session Duration**: ~1.5 hours
**Status**: ✅ Complete
**Key Files Changed**: `components/BuyMeCoffeeWidget.js`, `components/ActionFABs.js`

### 🎯 Key Outcomes

1.  **Premium Glassmorphism Upgrade**:
    *   **BMC FAB**: Transformed from a standard button to a square, dark-glass (90% opacity), iOS-style floating action button.
    *   **Action FABs**: Applied matching dark-glass styles to the main "+" button and "Request/Report" menu items.
    *   **Visibility Balance**: Tuned opacity to **0.9** (90%) to ensure visibility against busy backgrounds while maintaining the "glass" aesthetic.

2.  **Reliability Improvements**:
    *   **Click Fix**: Replaced complex `onClick` handlers with native `<a>` tags for the BMC widget to ensure 100% reliable navigation.
    *   **Z-Index Strategy**: Assigned `z-index: 9999` to BMC widget and `z-index: 50` to ActionFABs to perform proper layering (BMC > FAB Menu).

### 🐛 Bugs Logged & Fixed

| Bug | Root Cause | Fix |
|-----|------------|-----|
| **BMC Click Inconsistency** | `onClick` handler conflict & z-index overlay issues | Switched to native `<a>` tag with `pointer-events: auto` and `z-index: 9999 !important`. |
| **"Invisible" Buttons** | Transparency set too high (10-20% opacity) | Increased background opacity to **90%** (`rgba(10, 10, 10, 0.9)`). |
| **JSX Syntax Errors** | Broken code blocks during refactoring | Consolidated duplicate code blocks and fixed closing tags stringently. |

### 📚 Lessons Learned

1.  **Glassmorphism vs. Usability**: True transparency (<50%) looks great in mockups but fails in production on busy backgrounds. **90% opacity** is the sweet spot for "functional glass".
2.  **The "Z-Index War"**: Interactive elements (FABs) must have explicit, high z-indices (`9999`) if they float over other complex layers like sticky headers or other FABs.

---

## 2026-01-27: Homepage Support Banner with BuyMeACoffee Integration

**Session Duration**: ~45 minutes
**Status**: ✅ Complete (Pending Push)
**Key Files Changed**: 6 files (2 created, 3 modified, 1 deleted)

### 🎯 Key Outcomes

1. **New Support Banner**: Created `HomepageBanner.js` - a premium glassmorphism modal asking users to support CineAmore via BuyMeACoffee.
2. **BuyMeACoffee Widget**: Added floating BMC button widget on LEFT side of all pages (avoiding existing FABs on right).
3. **Footer Integration**: Added permanent "☕ Support" link in footer navigation.
4. **Old Banner Removed**: Deleted legacy `PromoBanner.js` donation banner.

### 📦 Files Changed

| File | Action | Purpose |
|------|--------|---------|
| `components/HomepageBanner.js` | **Created** | New support banner with 5+5 countdown |
| `components/BuyMeCoffeeWidget.js` | **Created** | Floating BMC widget for all pages |
| `app/layout.js` | **Modified** | Added BMC widget globally |
| `app/page.js` | **Modified** | Uses new banner, removed conditional |
| `components/Footer.js` | **Modified** | Added permanent support link |
| `components/PromoBanner.js` | **Deleted** | Legacy donation banner removed |

### ⚡ Banner Features

- **Title**: "Help Keep CineAmore Running" with ☕💛 icon
- **Countdown System**: 
  - 0-5s: No close option
  - 5-10s: X button appears
  - 10s+: "Access CineAmore" button appears
- **Reappearance**: Every 24 hours (localStorage timestamp)
- **Content**: Appeal message, BuyMeaCoffee button, Twitter & Telegram links
- **Style**: Glassmorphism modal with gold/amber BMC theming

### 🔧 Technical Notes

- BMC Widget positioned on LEFT (`data-position="Left"`) to avoid FAB overlap
- Uses `localStorage.setItem('supportBanner_lastDismissed', Date.now())` for 24h tracking
- Body scroll locked while banner is visible
- Mobile-responsive design maintained

---

## 2026-01-25: Search Feature Parity + Homepage Genre Expansion
...
1. **Search Feature for Series/Anime Modes**: Fixed search parity between Movies mode and Series/Anime modes. Series and Anime now support Enter-key search like Movies.
2. **More Homepage Genres**: Expanded genre rows from 12 to 20 on the homepage.

### ✅ Features Completed

#### 1. Enter-Key Search for Series & Anime
- **Problem**: Movies mode supported Enter-key filtering to show grid results, but Series/Anime only had dropdown typeahead.
- **Solution**: Implemented hybrid approach - kept dropdown for quick access + added Enter-key navigation to dedicated search pages.
- **Files Created**:
  - `app/series/search/page.js` - Series search results grid
  - `app/anime/search/page.js` - Anime search results grid (filters for Japanese animation)
- **Files Modified**:
  - `components/SeriesGrid.js` - Added `useRouter`, `handleSearchSubmit` for Enter key
  - `components/AnimeHero.js` - Added `useRouter`, `handleSearchSubmit` for Enter key

#### 2. Homepage Genre Expansion
- **Problem**: Homepage showed only 12 genre rows.
- **Solution**: Expanded `HOME_GENRES` array in `app/page.js` to include 8 additional genres.
- **Genres Added**: War, History, Music, Documentary, Family, Western, Sport, Biography

### 📦 Commits
| Hash | Message |
|------|---------|
| `8d52ed4` | feat: Add Enter key search functionality for Series and Anime modes |
| `c374050` | feat: Add more genre rows to homepage |
